...
export default function InboxPage() {
  const { role, isAdmin, isAgent } = useAuth();
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatusCard label="Total Messages" value={`${totalConversations}`} icon={MessageCircle} />
            <StatusCard
              label="Unread"
              value={`${unreadCount}`}
              icon={Clock}
                    ...
                    >
                      <div className="flex justify-between items-start gap-3 mb-2">
                        ...
                        <div
                          ...
                          className="text-slate-400 hover:text-yellow-500 transition-colors flex-shrink-0 cursor-pointer p-1 hover:bg-slate-100 rounded"
                        >
                          <Star
                            className={`w-4 h-4 ${
                              message.starred
              ...
              <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[600px]">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  <div className="flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#0F9D58]/20 flex items-center justify-center flex-shrink-0">
                      <span className="text-xs font-bold text-[#0F9D58]">
                        {messages.find((m) => m.id === selectedMessage)?.avatar || 'C'}
                      </span>
                    </div>
                    <div className="flex-1">
                      <div className="bg-slate-100 rounded-lg p-3 mb-1">
                        <p className="text-sm text-slate-900">
                          Hi, I have a question about my recent order. Can you help me?
                        </p>
                      </div>
                ...
                <div className="p-4 border-t border-slate-200">
                  <div className="flex gap-2">
                    ...
                    />
                    <button className="px-4 py-2 bg-[#0F9D58] text-white rounded-lg hover:bg-green-600 transition-colors font-medium">
                      Send
                    </button>
                  </div>
                </div>
...
function StatusCard({ label, value, icon: Icon, highlight }: StatusCardProps) {
  return (
    ...
    >
      <p className={`text-xs font-medium mb-1 ${highlight ? 'text-blue-600' : 'text-slate-600'}`}>
        {label}
      </p>
